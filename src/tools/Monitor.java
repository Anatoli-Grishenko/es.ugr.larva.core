...
//        String res = getHexaKey(size);
    ...
    public static boolean isHiddenMonitor(ACLMessage msg) {
        if (msg == null) {
            return false;
        } else {
    ...
    public static ACLMessage hideSubscribe(ACLMessage msg, NetworkAccessPoint nap) {
    ...
    public static void plainSubscribe(LARVABaseAgent ba, NetworkAccessPoint nap, MonitorBox mb) {
    ...
    public static ACLMessage hidePong(ACLMessage incoming, ACLMessage outgoing, NetworkAccessPoint nap) {
        return hidePong(incoming, outgoing, nap, "");
    }

    public static ACLMessage hidePong(ACLMessage incoming, ACLMessage outgoing, NetworkAccessPoint nap, String description) {
        ...
        try {
            if (nc != null) {
                if (nc.gettReceive().length() > 0 && nc.getAccessPoint().length() > 0) {
                    ...
                    Ole.oleToObject(oNap, nap, NetworkAccessPoint.class);
...
//            NetworkAccessPoint nap = new NetworkAccessPoint();
////            Ole.oleToObject(new Ole(mync.getAccessPoint()), nap, NetworkAccessPoint.class);
