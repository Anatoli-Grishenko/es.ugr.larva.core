...
public class LARVAAirTrafficControl3D {
    ...
    public void setMap(Color cmap[][], Palette p) {
        ...
        String3D s3d;
        dpMap3D.addObject3D(new Line3D(new Point(0, 0, 0), new Point(wi, 0, 0)).setColor(Color.GREEN).setName("EjeX"));
        dpMap3D.addObject3D(new Line3D(new Point(0, 0, 0), new Point(0, he, 0)).setColor(Color.BLUE).setName("EjeY"));
        dpMap3D.addObject3D(new Line3D(new Point(0, 0, 0), new Point(0, 0, wi)).setColor(Color.RED).setName("EjeZ"));
        
        s3d = new String3D("X");
        s3d.setPosition(new Point(wi, 0, 0));
        s3d.setColor(Color.GREEN);
        dpMap3D.addObject3D(s3d);
        s3d = new String3D("Y");
        s3d.setPosition(new Point(0, he, 0));
        s3d.setColor(Color.BLUE);
        dpMap3D.addObject3D(s3d);
        s3d = new String3D("Z");
        s3d.setPosition(new Point(0, 0,wi));
        ...
        for (int i = 0; i < dimx - 1; i++) {
            for (int j = 0; j < dimy - 1; j++) {
                ...
                Polygon3D poly = new Polygon3D(new Point(x * step, y * step, alt)).
                        setFilled(true).
                        addVertex(new Point(x * step + step, y * step, alt2)).
                        addVertex(new Point(x * step + step, y * step + step, alt3)).
                        addVertex(new Point(x * step, y * step + step, alt4)).
                        addVertex(new Point(x * step, y * step, alt));
                poly.setColor(c).setName("OBJ-" + i + "-" + j);
                dpMap3D.addObject3D(poly);
                if (i == 0) {
                    dpMap3D.addObject3D(new Polygon3D(new Point(x * step, y * step, alt)).
                            setFilled(true).
                            addVertex(new Point(x * step, y * step + step, alt4)).
                            addVertex(new Point(x * step, y * step + step, 0)).
                            addVertex(new Point(x * step, y * step, 0)).
                            addVertex(new Point(x * step, y * step, alt)).setColor(Color.ORANGE)
                    );
                }
                if (i == dimx - 2) {
                    dpMap3D.addObject3D(new Polygon3D(new Point(x * step + step, y * step, alt2)).
                            setFilled(true).
                            addVertex(new Point(x * step + step, y * step + step, alt3)).
                            addVertex(new Point(x * step + step, y * step + step, 0)).
                            addVertex(new Point(x * step + step, y * step, 0)).
                            addVertex(new Point(x * step + step, y * step, alt2)).setColor(Color.ORANGE)
                    );
                }
                if (j == 0) {
                    dpMap3D.addObject3D(new Polygon3D(new Point(x * step, y * step, alt)).
                ...
                if (j == dimy - 2) {
                    dpMap3D.addObject3D(new Polygon3D(new Point(x * step, y * step + step, alt4)).
                            setFilled(true).
                            addVertex(new Point(x * step + step, y * step + step, alt3)).
                            addVertex(new Point(x * step + step, y * step + step, 0)).
                            addVertex(new Point(x * step, y * step + step, 0)).
                            addVertex(new Point(x * step, y * step + step, alt4)).setColor(Color.ORANGE)
                    );
                }
                if (i == dimy - j) {
                    Polygon3D poly2 = new Polygon3D(new Point(x * step, y * step, alt5)).
                            setFilled(true).
                            addVertex(new Point(x * step + step, y * step, alt5)).
                            addVertex(new Point(x * step + step, y * step + step, alt5)).
                            addVertex(new Point(x * step, y * step + step, alt5)).
                            addVertex(new Point(x * step, y * step, alt5));
                }
            }
        }
        JScrollBar sbaux;
    ...
    public void addTrail(String ID, int x, int y, int z) {
        ...
        Point p = new Point((x-dimx/2)*step, (y-dimy/2)*step, z+10);
